...
class ChangePassword: UIViewController,UITextFieldDelegate {
    ...
    @objc func validationBeforeChangePassword() {
        ...
        if self.txtNewPassword.text == "" {
            ...
            if let loadedString = UserDefaults.standard.string(forKey: "key_save_password") {
                print(loadedString)
                
                if String(loadedString) == String(self.txtConfirmPassword.text!) {
                    //
                    print("new password should be different from previous password")
                    let alert = UIAlertController(title: String("Error!"), message: String("New password should be different from your last password."), preferredStyle: UIAlertController.Style.alert)
                    alert.addAction(UIAlertAction(title: "Ok", style: UIAlertAction.Style.default, handler: { action in
