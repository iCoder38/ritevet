//
//  edit_bank_info.swift
//  RiteVet
//
//  Created by Dishant Rajput on 27/02/24.
...
class edit_bank_info: UIViewController, UITextFieldDelegate {
...
//    }
    ...
    @objc func edit_profile_bank_info_seller() {
        
        let indexPath = IndexPath.init(row: 0, section: 0)
        let cell = tbleView.cellForRow(at: indexPath) as! edit_bank_info_table_cell
        ...
        if let person = UserDefaults.standard.value(forKey: "keyLoginFullData") as? [String:Any] {
            ...
            parameters = [
                ...
                "accountNo"     :   String(cell.txtBankAccountNumber.text!),
                "RoutingNo"     :   String(cell.txtRoutingCode.text!),
                "swiftNumber"   :   String(cell.txtSwitchNumberForAccounts.text!),
                "BankName"      :   String(cell.txtSelectBank.text!),
                "accountType"   :   String(cell.txtAccountType.text!),
            ]
        }
            ...
            case .success(_):
                if let data = response.value {
                    ...
                    if strSuccess == "success" {
                        ...
                        let alert = UIAlertController(title: "Alert", message: JSON["msg"]as Any as? String, preferredStyle: .alert)
...
//
// MARK: - APPLE PAY -
//extension edit_bank_info: PKPaymentAuthorizationViewControllerDelegate {
//    
...
extension edit_bank_info: UITableViewDataSource , UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell:edit_bank_info_table_cell = tableView.dequeueReusableCell(withIdentifier: "edit_bank_info_table_cell") as! edit_bank_info_table_cell
        ...
        /****** FINISH BUTTON *********/
        Utils.buttonDR(button: cell.btnFinish, text: "SUBMIT", backgroundColor: BUTTON_BACKGROUND_COLOR_BLUE, textColor: BUTTON_TEXT_COLOR, cornerRadius: 20)
        cell.btnFinish.addTarget(self, action: #selector(edit_profile_bank_info_seller), for: .touchUpInside)
        
        if let person = UserDefaults.standard.value(forKey: "keyLoginFullData") as? [String:Any] {
            print(person)
            
            cell.txtYourName.text                   = (person["AccountName"] as! String)
            cell.txtSelectBank.text                 = (person["BankName"] as! String)
            cell.txtBankAccountNumber.text          = (person["accountNo"] as! String)
            cell.txtRoutingCode.text                = (person["RoutingNo"] as! String)
            cell.txtAccountType.text                = (person["accountType"] as! String)
            cell.txtSwitchNumberForAccounts.text    = (person["swiftNumber"] as! String)
...
//        let cell = tbleView.cellForRow(at: NSIndexPath(row: 0, section: 0) as IndexPath) as! edit_bank_info_table_cell
//        
//        if cell.txtYourName.text == "" {
//            CRNotifications.showNotification(type: CRNotifications.error, title: "Error!", message:"Field Should not be empty.", dismissDelay: 1.5, completion:{})
//        }
...
class edit_bank_info_table_cell: UITableViewCell {
